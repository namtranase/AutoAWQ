...
class BaseAWQForCausalLM(nn.Module):
    ...
    def quantize(self, tokenizer=None, quant_config={},
                       ...
                       split="train", text_column="text", duo_scaling=True, modules_to_not_convert=None, save_scaled_model=False):
        self.quant_config: AwqConfig = AwqConfig.from_dict(quant_config)

        quantizer = AwqQuantizer(
            self, self.model, tokenizer, self.quant_config.w_bit, self.quant_config.q_group_size,
            self.quant_config.version, calib_data, split, text_column, duo_scaling, save_scaled_model, modules_to_not_convert=modules_to_not_convert
        )
        quantizer.quantize()
